...
object AngConfigManager {
    ...
    /**
     * Outcome of [refreshSimpleServerList], distinguishing "server sent an empty/invalid
     * list on purpose" from "we couldn't reach the server at all" so callers can apply the
     * two very different cache policies the simplified Free VPN client needs.
     */
    sealed class SimpleRefreshResult {
        /** Downloaded successfully and at least one valid config was parsed. */
        data class Success(val configCount: Int) : SimpleRefreshResult()

        /** Downloaded successfully (HTTP ok) but the file was empty or had no valid config. */
        data object EmptyOrInvalid : SimpleRefreshResult()

        /** Could not download the file at all (timeout, no internet, temporary server error). */
        data object NetworkError : SimpleRefreshResult()
    }

    /**
     * Simplified subscription refresh used by the "Free VPN" client screen
     * (see SimpleVpnRepository). Unlike [updateConfigViaSub], this distinguishes a
     * successful-but-empty response from a network failure, and always replaces the
     * previous server list rather than silently keeping it when nothing new is found.
     *
     * @param it The subscription item pointing at RemoteConfig.SERVER_LIST_URL.
     */
    fun refreshSimpleServerList(it: SubscriptionCache): SimpleRefreshResult {
        try {
            val url = HttpUtil.toIdnUrl(it.subscription.url)
            if (!Utils.isValidUrl(url)) {
                return SimpleRefreshResult.NetworkError
            }

            // null = the request itself failed (timeout / no internet / server unreachable).
            // A non-null value, even "", means we got an actual HTTP response.
            val configText: String? = try {
                HttpUtil.getUrlContentWithUserAgent(
                    UrlContentRequest(
                        url = url,
                        timeout = RemoteConfig.REQUEST_TIMEOUT_SECONDS * 1000
                    )
                )
            } catch (e: Exception) {
                LogUtil.e(AppConfig.TAG, "refreshSimpleServerList: failed to fetch server list", e)
                null
            }

            if (configText == null) {
                // Genuine network failure: keep whatever cache the caller has, per policy.
                return SimpleRefreshResult.NetworkError
            }

            if (configText.isBlank()) {
                // Got a response, it was just empty: the operator intentionally cleared the list.
                LogUtil.i(AppConfig.TAG, "refreshSimpleServerList: server returned an empty file")
                return SimpleRefreshResult.EmptyOrInvalid
            }
            ...
            return if (count > 0) {
                it.subscription.lastUpdated = System.currentTimeMillis()
                MmkvManager.encodeSubscription(it.guid, it.subscription)
                LogUtil.i(AppConfig.TAG, "refreshSimpleServerList: updated ${count} configs")
                SimpleRefreshResult.Success(count)
            } else {
                // We got a response, it just had nothing usable in it.
                LogUtil.i(AppConfig.TAG, "refreshSimpleServerList: response had no valid configs")
                SimpleRefreshResult.EmptyOrInvalid
            }
        } catch (e: Exception) {
            LogUtil.e(AppConfig.TAG, "refreshSimpleServerList: unexpected failure", e)
            return SimpleRefreshResult.NetworkError
        }
    }
